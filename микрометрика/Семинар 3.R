...
# Микроэконометрика
# Семинар №3 - Выбор оптимальной спецификации модели бинарного выбора
#--------------
...
# В условиях нарушения допущения о распределении случайной ошибки получить состоятельные оценки параметров
# модели можно при помощи непараметрических методов, например. ядерной регрессии, предложенной Klein and Spady
# Расчеты будут осуществляться чрезвычайно долго и могут занять вплоть до часа.
# model_np <- npindex(work ~ age + I(age ^ 2) +         
#                     educ_1 + educ_2 + educ_3 +
#                     children_n_18 + marriage + I(marriage * children_n_18) +
#                     BMI,
#                     data=h[selection_condition,],
...
me_BMI <- pnorm(model_hetprobit_latent_adj) * 
